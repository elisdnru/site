...
class BlogCalendarWidget extends DWidget
{
    ...
    public function run()
    {
        ...
        $pnc = array('&lt;'=>CHtml::normalizeUrl(array('/blog/default/date', 'date'=>$prevMonth)),
            '&gt;'=>CHtml::normalizeUrl(array('/blog/default/date', 'date'=>$nextMonth)));
        ...
        // Make the links
        $posts = BlogPost::model()->cache(3600*24)->findAll(array(
            'condition'=>'date LIKE :date',
            'params'=>array(':date'=>"$year-$month-%"),
        ));

        foreach($posts as $post) {
            $days[date('j', strtotime($post->date))] = array(CHtml::normalizeUrl(array('/blog/default/date', 'date'=>date('Y-m-d', strtotime($post->date)))), 'linked-day');
        }

        if (isset($locale) && $locale == 'ja_JP.utf8') $len = 3;
        else $len = 2;

        $this->render('Calendar/calendar', array(
            'year'=>$year,
            'month'=>$month,
            'days'=>$days,
            'len'=>$len,
