...
if ($this->moduleAllowed('portfolio')) $this->admin[] = array('label'=>'Добавить работу', 'url'=>$this->createUrl('create', array('category'=>$category)));
...
<p>
    Раздел:  <?php echo CHtml::dropDownList('category', $category, array('0'=>'Все разделы') + PortfolioCategory::model()->getTabList(), array('onchange'=>'this.form.submit()')); ?>
    <?php echo CHtml::submitButton('Выбрать', array('class'=>'js_hide')); ?>
</p>
    <br />
...
$editurl = $this->createUrl('update', array('id'=>$item->id));
$delurl = $this->createUrl('delete', array('id'=>$item->id));

$toggle_public = $this->createUrl('toggle', array('id'=>$item->id, 'param'=>'public'));
