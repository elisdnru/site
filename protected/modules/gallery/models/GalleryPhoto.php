...
class GalleryPhoto extends CActiveRecord implements DICommentDepends
{
	...
	public function rules()
	{
		...
		return array(
            ...
            array('category_id', 'exist', 'className' => 'GalleryCategory', 'attributeName' => 'id'),
			array('public', 'numerical', 'integerOnly'=>true),
			array('date', 'date', 'format'=>'yyyy-MM-dd hh:mm:ss'),
			array('text, description, del_image', 'safe'),
			array('video', 'videoService', 'allowEmpty'=>true),
            array('title, image_alt, pagetitle, keywords', 'length', 'max'=>'255'),
	...
	public function videoService($attribute, $options)
	{
		...
		if (
			preg_match('#https?:\/\/(www\.)?youtube\.com/(watch\?v=|v/)(?<id>[\w\d_-]+)([&\?][&\w\d=_-]+)?#i', $this->$attribute, $matches) ||
    ...
    private function fillDefaultValues()
    {
        ...
        if (!$this->description) $this->description = strip_tags($this->title);
        if (!$this->image_alt) $this->image_alt = strip_tags($this->title);
    }
    ...
    public function getUrl(){
        if ($this->_url === null)
		{
			...
			$this->_url = Yii::app()->createUrl('/gallery/photo/show', array('id'=>$this->getPrimaryKey()));
		}
        return $this->_url;
    }
