...
app-clear:
	docker run --rm -v "${PWD}":/app -w /app dockerhub.timeweb.cloud/alpine:3.23 sh -c 'rm -rf .ready var/* public/assets/* public/build/* public/upload/* tests/_output/* tests/_support/_generated/*'
...
app-permissions:
	docker run --rm -v "${PWD}":/app -w /app dockerhub.timeweb.cloud/alpine:3.23 sh -c 'mkdir -p public/build && chmod 777 var public/assets public/build public/upload tests/_output tests/_support/_generated'
...
app-fixtures:
	...
	docker run --rm -v "${PWD}":/app -w /app dockerhub.timeweb.cloud/alpine:3.23 sh -c 'rm -rf public/upload/* && cp -rf demo/upload/* public/upload'
	docker run --rm -v "${PWD}":/app -w /app dockerhub.timeweb.cloud/alpine:3.23 sh -c 'find public/upload -type d -exec chmod 777 {} \;'
	docker run --rm -v "${PWD}":/app -w /app dockerhub.timeweb.cloud/alpine:3.23 sh -c 'find public/upload -type f -exec chmod 666 {} \;'

app-assets-build:
	docker compose run --rm site-node-cli yarn build

app-ready:
	docker run --rm -v "${PWD}":/app --workdir=/app --user ${UID}:${GID} dockerhub.timeweb.cloud/alpine:3.23 touch .ready
