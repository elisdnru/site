...
app-clear:
	docker run --rm -v ${PWD}:/app -w /app alpine:3.22 sh -c 'rm -rf .ready var/* public/assets/* public/build/* public/upload/* tests/_output/* tests/_support/_generated/*'
...
app-permissions:
	docker run --rm -v ${PWD}:/app -w /app alpine:3.22 sh -c 'mkdir -p public/build && chmod 777 var public/assets public/build public/upload tests/_output tests/_support/_generated'
...
app-fixtures:
	...
	docker run --rm -v ${PWD}:/app -w /app alpine:3.22 sh -c 'rm -rf public/upload/* && cp -rf demo/upload/* public/upload'
	docker run --rm -v ${PWD}:/app -w /app alpine:3.22 sh -c 'find public/upload -type d -exec chmod 777 {} \;'
	docker run --rm -v ${PWD}:/app -w /app alpine:3.22 sh -c 'find public/upload -type f -exec chmod 666 {} \;'
