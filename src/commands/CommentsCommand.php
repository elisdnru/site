	...
	public function actionVacuum() {
		...
		foreach ($rows as $i=>$row) {
			...
			if ($old_id != $new_id) {
				...
				$success = $db->createCommand('UPDATE {{comment}} SET id = :new_id WHERE id = :old_id')->execute(array(
					'old_id'=>$old_id,
					'new_id'=>$new_id,
				)) && $success;

				$db->createCommand('UPDATE {{comment}} SET parent_id = :new_id WHERE parent_id = :old_id')->execute(array(
					'old_id'=>$old_id,
					'new_id'=>$new_id,
				));
