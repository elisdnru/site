...
class Email extends CApplicationComponent {
	...
	public function send($arg1=null) {
		if ($this->view !== null) {
			...
			$view = Yii::app()->controller->renderPartial('//email/'.$this->view, array_merge($vars, array('email'=>$this)), true);
			if ($this->layout === null) {
				$message = $view;
			} else {
				$message = Yii::app()->controller->renderPartial('//email/layouts/'.$this->layout, array('content'=>$view), true);
			}
		} else {
			if ($arg1 === null) {
