...
class Email extends CApplicationComponent
{
    ...
    public function send($arg1 = null)
    {
        if ($this->view !== null) {
            ...
            $view = Yii::app()->controller->renderPartial('//email/' . $this->view, array_merge($vars, ['email' => $this]), true);
            if ($this->layout === null) {
                $message = $view;
            } else {
                $message = Yii::app()->controller->renderPartial('//email/layouts/' . $this->layout, ['content' => $view], true);
            }
        } else {
            if ($arg1 === null) {
    ...
    private function mail($to, $subject, $message)
    {
        switch ($this->delivery) {
            ...
            case 'debug':
                $debug = Yii::app()->controller->renderPartial(
                    'email.debug',
                    array_merge(compact('to', 'subject', 'message'), ['headers' => $this->createHeaders()]),
                    true
                );
