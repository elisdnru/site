...
class MultilingualBehavior extends CActiveRecordBehavior
{
    ...
    private function createLocalizedRelation($owner, $lang)
    {
        ...
        $owner->getMetaData()->relations[$this->localizedRelation] = new $class($this->localizedRelation, $this->langClassName, $this->langForeignKey, ['on' => $this->localizedRelation . "." . $this->langField . "='" . $lang . "'", 'index' => $this->langField]);
    }
    ...
    public function attach($owner)
    {
        ...
        $simple_table_name = str_replace(['{{', '}}'], '', array_pop($table_name_chunks));
        ...
        $owner->getMetaData()->relations[$this->multilangRelation] = new $class($this->multilangRelation, $this->langClassName, $this->langForeignKey, ['index' => $this->langField]);
    ...
    public function afterFind($event)
    {
        ...
        } elseif ($owner->hasRelated($this->localizedRelation)) {
    ...
    public function afterSave($event)
    {
        ...
        if (!$main_owner->isNewRecord) {
            $model = call_user_func([$this->langClassName, 'model']);
            $c = new CdbCriteria();
    ...
    public function afterDelete($event)
    {
        if ($this->forceDelete) {
            ...
            $model = call_user_func([$this->langClassName, 'model']);
            $model->deleteAll("{$this->langForeignKey}=:id", ['id' => $ownerPk]);
        }
    }
