...
class EMailer
{
    ...
    public function __call($method, $params)
    {
        if (is_object($this->_myMailer) && get_class($this->_myMailer) === 'PHPMailer') {
            return call_user_func_array([$this->_myMailer, $method], $params);
        } else {
     ...
     * @param string $layout
     */
    public function getView($view, $vars = [], $layout = null)
    {
        $body = Yii::app()->controller->renderPartial($this->pathViews . '/' . $view, array_merge($vars, ['content' => $this->_myMailer]), true);
        if ($layout === null) {
            $this->_myMailer->Body = $body;
        } else {
            $this->_myMailer->Body = Yii::app()->controller->renderPartial($this->pathLayouts . '/' . $layout, ['content' => $body], true);
        }
    }
}
