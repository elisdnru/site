...
function createFolder(location) {
    name = document.browse.foldername.value;
    document.location.href = location + name;
}
...
rowHighlight = function () {
    ...
    for (var ii = 0; ii < y.length; ii++) {
        y[ii].onmouseover = function () {
            if (this.className != "nohover") this.className = "over " + this.className;
        }
        y[ii].onmouseout = function () {
            this.className = this.className.replace("over", "");
            this.className = this.className.replace(" ", "");
        }
    }
