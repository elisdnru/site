...
<article class="comment<?= !$data->moder ? ' newcomment' : '' ?><?= !$data->public ? ' nopublcomment' : '' ?>" id="comment_<?= $data->id ?>">
    ...
    <header>
        ...
        <span class="like">
            <span id="like-<?= $data->id ?>"><?= $data->likes ?></span>
            <a rel="nofollow" class="ajax-load like-icon<?= $data->liked ? ' like-active' : '' ?>" data-load="like-<?= $data->id ?>" href="<?= Url::to(['/comment/ajax/like', 'id' => $data->id]) ?>" title="Мне нравится"></a>
        </span>
    ...
    <div class="text">
        ...
        <p>
            <a href="<?= $data->getUrl() ?>"><?= Html::encode($data->material->getCommentTitle()) ?></a>
        </p>
    </div>
